...
/**
 * Uploads a file to R2 under the 'b2b_clients' directory.
 * Returns the file key (relative path).
 */
export const uploadAttachment = async (file: File): Promise<string> => {
  console.log('Starting upload for:', file.name);
  const fileKey = await uploadFileToR2(file, BUCKET, 'b2b_clients/attachments');
...
export const deleteAttachment = async (filePath: string): Promise<void> => {
  const cleanPath = filePath
    .replace(/^\//, '')
    .replace(/^b2b_clients\/attachments\/b2b_clients\/attachments\//, 'b2b_clients/attachments/');
