...
// Constants
const FOLDER_ID = '1X1jcsX531t-ovk0_1zegFZg2GOB_gYak';

const Playbook: React.FC<PlaybookProps> = ({ session }) => {
  const [slides, setSlides] = useState<GoogleSlide[]>([]);
  const [error, setError] = useState<string>('');
  const [isLoading, setIsLoading] = useState(true);

  useEffect(() => {
    console.log('Session in Playbook:', session);
  ...
  return (
    <div className="w-full py-4">
      <h1 className="text-2xl font-semibold text-gray-900 mb-6">Playbook</h1>
      {error ? (
