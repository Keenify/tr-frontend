                            ...
                            <tbody>
                                {priceTiers.map((tier) => (
                                    <tr key={tier.id} className={`hover:bg-gray-50 ${isEditing === tier.id ? 'bg-blue-50' : ''}`}>
                                        {isEditing === tier.id ? (
                                            <>
                                                <td className="border p-1"><input type="text" name="tier_name" value={editFormData.tier_name ?? ''} onChange={(e) => handleInputChange(e, tier.id)} className="w-full p-1 border rounded text-sm" aria-label="Edit Tier Name"/></td>
                                                <td className="border p-1"><input type="number" step="0.01" name="fob_price_per_carton" value={editFormData.fob_price_per_carton ?? 0} onChange={(e) => handleInputChange(e, tier.id)} className="w-24 p-1 border rounded text-right text-sm" aria-label="Edit FOB Price Per Carton"/></td>
                                                <td className="border p-1"><input type="number" step="0.01" name="fob_price_per_unit" value={editFormData.fob_price_per_unit ?? 0} onChange={(e) => handleInputChange(e, tier.id)} className="w-24 p-1 border rounded text-right text-sm" aria-label="Edit FOB Price Per Unit"/></td>
                                                <td className="border p-1"><input type="number" step="0.01" name="recommended_rrp" value={editFormData.recommended_rrp ?? 0} onChange={(e) => handleInputChange(e, tier.id)} className="w-24 p-1 border rounded text-right text-sm" aria-label="Edit Recommended RRP"/></td>
