...
export const ExportPriceTierModal: React.FC<ExportPriceTierModalProps> = ({
    ...
    const [editFormData, setEditFormData] = useState<Partial<Omit<ProductExportPriceTier, 'fob_price_per_carton' | 'fob_price_per_unit' | 'recommended_rrp'> & { fob_price_per_carton: string, fob_price_per_unit: string, recommended_rrp: string }>>({});
    const [editFormErrors, setEditFormErrors] = useState<TierFormErrors>({});
    const [newTierData, setNewTierData] = useState<Omit<CreateProductExportPriceTierRequest, 'product_id' | 'fob_price_per_carton' | 'fob_price_per_unit' | 'recommended_rrp'> & { fob_price_per_carton: string, fob_price_per_unit: string, recommended_rrp: string }>({
        tier_name: '',
    ...
    useEffect(() => {
     ...
     const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>, id: number | null = null) => {
        ...
        const isNumericField = ['fob_price_per_carton', 'fob_price_per_unit', 'recommended_rrp'].includes(name);
    ...
    const isFormValid = (): boolean => {
        ...
        return allFieldsFilled && allNumericValid;
    };

    const handleAddTier = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!productId || hasErrors(newTierErrors)) {
        ...
        try {
            ...
            // Convert valid strings to numbers, default invalid/empty to 0
            ...
            if (fobCarton < 0 || fobUnit < 0 || rrp < 0) {
    ...
    const handleUpdateTier = async (id: number) => {
        if (!editFormData || hasErrors(editFormErrors)) {
            setError("Please fix the errors in the form.");
            return;
        }
        setIsLoading(true);
