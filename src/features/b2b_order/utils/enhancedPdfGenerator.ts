...
export const generateB2BOrderPDFEnhanced = (rows: B2BOrderRow[], companyInfo?: any) => {
  ...
  if (companyInfo) {
    ...
    logoImg.onload = () => {
      ...
      // Add B2B Order information
      doc.setFontSize(14);
      doc.text('B2B Order', 10, 50);
      doc.setFontSize(12);
      doc.text(`Updated At: ${new Date().toLocaleDateString()}`, 10, 55);
  ...
  function generateProductTable(doc: jsPDF, rows: B2BOrderRow[]) {
