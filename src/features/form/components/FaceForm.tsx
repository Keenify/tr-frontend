...
import { FaceFormValues, FaceFormRow, BusinessUnitRow as BusinessUnitRowType } from '../types/faceFormTypes';
  ...
  const onSubmit = async (values: FaceFormValues) => {
    ...
    try {
      ...
      const functionRows = values.functions
        .filter(row => row.accountable_employee_id && row.function_name.trim()) // Only submit rows with required data
        .map((row) => ({
          company_id: companyInfo.id,
      ...
      const businessUnitRows = values.business_units
        .filter(row => row.head_employee_id && row.business_unit_name.trim()) // Only submit rows with required data
        .map((row) => ({
          company_id: companyInfo.id,
      ...
      if (allRows.length === 0) {
        setSubmitStatus('Please fill in at least one function or business unit with required information');
        return;
      }
      
      // Check if at least one function has both function name and accountable person
      const hasValidFunction = functionRows.some(row => 
        row.function_name.trim() && row.accountable_employee_id
      );
      
      if (!hasValidFunction && businessUnitRows.length === 0) {
        setSubmitStatus('Please fill in at least one function with both function name and person accountable');
        return;
      }
